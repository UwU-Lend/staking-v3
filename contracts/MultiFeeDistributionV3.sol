...
contract MultiFeeDistributionV3 is ERC721Holder, Ownable {
  ...
  function setMinters(address[] calldata _minters) external onlyOwner {
    for (uint256 i = 0; i < minters.length(); i++) {
      require(minters.remove(minters.at(i)), 'Fail to remove minter');
    }
  ...
  function accountLiquidity(address account) public view returns(
    uint256 total,
    uint256 locked,
    uint256 unlockable
  ) {
  ...
  function lock(uint256[] calldata nftIds) external {
    ...
    for (uint256 i = 0; i < nftIds.length; i++) {
      ...
      require(tickRange[1] >= pos.tickLower, 'Exceeded upper tick range');
