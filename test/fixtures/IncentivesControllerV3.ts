...
export const incentivesControllerV3Fixture = async (): Promise<IncentivesControllerV3FixtureResult> => {
  ...
  const controllerV3 = await IncentivesControllerV3.deploy(POOL_CONFIGURATOR, distributor.address, controllerV2.address);
  return { controllerV1, controllerV2, controllerV3, distributor, rewardToken, rewardTokenHolder };
}